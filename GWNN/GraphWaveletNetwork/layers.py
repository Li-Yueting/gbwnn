...
class Wavelet_Convolution(Layer):
    """Graph convolution layer."""
    def __init__(self, node_num,weight_normalize,input_dim, output_dim, placeholders,kernel, dropout=0.,
                 sparse_inputs=False, act=tf.nn.relu, bias=False,
                 featureless=False, **kwargs):
        ...
        self.act = act
        if kernel == 'low':
            self.support = placeholders['support'][0:2]
        elif kernel=='band':
            self.support = placeholders['support'][2:4]
        # self.support = placeholders['support']
        self.sparse_inputs = sparse_inputs
        self.featureless = featureless
        self.bias = bias
